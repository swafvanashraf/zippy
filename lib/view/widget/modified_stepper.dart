...
typedef ControlsWidgetBuilder = Widget Function(BuildContext context, ControlDetails details);

/// A builder that creates the icon widget for the [ModifiedStep] at [stepIndex], given
/// [stepState].
...
class ModifiedStepper extends StatefulWidget {
  ...
  ///
  /// This callback which takes in a context and a [ControlDetails] object, which
  ...
  /// [ControlDetails.currentStep] value within the callback can change the text
  ...
  @override
  State<ModifiedStepper> createState() => _ModifiedStepperState();
}

class _ModifiedStepperState extends State<ModifiedStepper> with TickerProviderStateMixin {
  ...
  bool _isLabel() {
    for (final ModifiedStep step in widget.steps) {
      if (step.label != null) {
        return true;
      }
    }
